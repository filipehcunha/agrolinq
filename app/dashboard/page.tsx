...
export default function DashboardPage() {
    const [viewMode, setViewMode] = useState<'produtor' | 'consumidor'>('produtor');

    return (
        <div className="min-h-screen bg-slate-50 flex flex-col">
            <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between shadow-sm sticky top-0 z-10">
                <div className="flex items-center gap-6">
                    <h1 className="text-xl font-bold text-green-600">AGROLINQ</h1>
                    
                    {/* View Toggle Dropdown */}
                    <div className="relative group">
                        <select 
                            value={viewMode}
                            onChange={(e) => setViewMode(e.target.value as 'produtor' | 'consumidor')}
                            className="appearance-none bg-green-50 border border-green-200 text-green-700 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5 pr-8 cursor-pointer font-medium"
                        >
                            <option value="produtor">Visão: Produtor</option>
                            <option value="consumidor">Visão: Consumidor</option>
                        </select>
                ...
                <div className="flex items-center gap-4">
                    <span className="text-sm text-gray-600 hidden md:inline">Bem-vindo(a), <span className="font-semibold">Usuário</span></span>
            ...
            <main className="flex-1 p-6">
                <div className="max-w-6xl mx-auto">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-800">
                                {viewMode === 'produtor' ? 'Painel do Produtor' : 'Portal do Consumidor'}
                            </h2>
                            <p className="text-gray-500 text-sm">
                                {viewMode === 'produtor' 
                                    ? 'Gerencie seus produtos, vendas e entregas.' 
                    ...
                    {viewMode === 'produtor' ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 transition-all hover:shadow-md">
                                ...
                                <p className="text-xs text-green-600 mt-2 font-medium">+0% em relação ao mês anterior</p>
                            </div>
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 transition-all hover:shadow-md">
                                ...
                                <p className="text-xs text-gray-400 mt-2">Nenhum pedido pendente</p>
                            </div>
                            ...
                            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 transition-all hover:shadow-md">
                                ...
                                <p className="text-3xl font-bold text-gray-900">0</p>
                                <p className="text-xs text-gray-400 mt-2">Sem conversas novas</p>
                            </div>
                        </div>
                    ) : (
                        /* Consumidor Content */
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                             <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 transition-all hover:shadow-md">
                                ...
                                <p className="text-3xl font-bold text-gray-900">0</p>
                                <p className="text-xs text-gray-400 mt-2">Você ainda não realizou pedidos</p>
                            </div>
                    ...
                    {/* common area for both? maybe recent activity */}
                    <div className="mt-12 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-center">
                            ...
                            <p className="text-gray-500">Nenhuma atividade para exibir no momento.</p>
                        </div>
                    </div>
                </div>
            </main>
